...
if ( ! function_exists( 'boldthemes_get_new_media_html' ) ) {
	function boldthemes_get_new_media_html( $arg = array() ) { 
		...
		if ( $video != '' ) {
			...
			$bt_link_bg_image = "";
			if ( $featured_image != '' ) $bt_link_bg_image .= " style='background-image: url(" . $featured_image . ")'";
			...
			$bt_quote_bg_image = "";
			if ( $featured_image != '' ) $bt_quote_bg_image .= " style='background-image: url(" . $featured_image . ")'";
		...
		if ( $share_facebook ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Facebook', 'law-firm' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,
			...
			if ( $share_twitter ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Twitter', 'law-firm' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,
						'color_scheme' => $color_scheme, 
			...
			if ( $share_linkedin ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Linkedin', 'law-firm' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size, 
						'color_scheme' => $color_scheme,
			...
			if ( $share_google_plus ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on Google Plus', 'law-firm' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size, 
						'color_scheme' => $color_scheme, 
			...
			if ( $share_vk ) {
				$share_html .= boldthemes_get_icon_html( 
					array ( 
						...
						'url_title' => esc_html__( 'Share on VK', 'law-firm' ), 
						'style' => $style,
						'shape' => $shape,
						'size' => $size,  
						'color_scheme' => $color_scheme,
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'law-firm' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'law-firm' ) );
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'law-firm' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'law-firm' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'law-firm' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'law-firm' ), $rating ) ?>">
									<span style="width:<?php echo wp_kses_post( ( $rating / 5 ) * 100 ); ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php esc_html_e( 'out of 5', 'law-firm' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'law-firm' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'law-firm' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'law-firm' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if ( ! class_exists( 'RWMB_BoldThemesText_Field' ) && class_exists( 'RWMB_Field' ) ) {
	class RWMB_BoldThemesText_Field extends RWMB_Field {
		...
		static function admin_enqueue_scripts() {			
			...
			wp_add_inline_script( 'boldthemes_text', $fake_customizer_js );
			wp_add_inline_script( 'boldthemes-text', $fake_customizer_js );
		}
			...
			if ( isset( BoldThemesFramework::$fake_customizer->control_arr[ $part_meta_key ] ) ) {
				if ( isset( BoldThemesFramework::$fake_customizer->control_arr[ $part_meta_key ]['type'] ) ) {
					...
					if ( $type == 'checkbox' ) {
						...
						$input = '<input type="text" class="boldthemes_value boldthemes_override_color_field" value="' . $meta_value . '">';
					} else if ( $type == 'WP_Customize_Image_Control' ) {
						$input = '<input type="text" class="boldthemes_value" value="' . $meta_value . '">';
						...
						if ( $src != '' ) {
							$input .= '<img src="' . $src . '"><span class="boldthemes_override_add_image"><i class="dashicons dashicons-plus-alt"></i></span>';
						} else {
...
if ( ! function_exists( 'boldthemes_get_posts_array_item' ) ) {
	function boldthemes_get_posts_array_item( $post_type = 'blog', $post_id, $post_author ) {
		...
		if ( $user_data ) {
			...
			$post_data['author'] = '<a href="' . esc_url_raw( $author_url ) . '" class="btArticleAuthorURL">' . esc_html( $author ) . '</a>';
...
if ( ! function_exists( 'boldthemes_set_global_uri' ) ) {
	function boldthemes_set_global_uri() {
		...
		$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'law-firm' ) . '\';';
		$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'law-firm' ) . '\';';
		return $data;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_date' ) ) {
	function boldthemes_get_post_date( $arg = array() ) {
		...
		if ( ! has_filter( 'get_the_time' ) ) {
			...
			return $prefix . get_the_time( 'Y-m-d G:m' ) . $suffix;
		}
		return $prefix . esc_html( date_i18n( BoldThemesFramework::$date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . $suffix;
	}
}
...
if ( ! function_exists( 'boldthemes_get_post_author' ) ) {
	function boldthemes_get_post_author( $author_url = false, $prefix = 'by', $show_avatar = false ) {
		...
		$output .= '<a href="' . esc_url_raw( $author_url ) . '" class="btArticleAuthorURL">' . $prefix . ' ' . esc_html( get_the_author_meta( 'display_name', $post_author_id )  ) . '</a>';
		$output .= '</span>';
		return $output;
	}
}
	...
	function boldthemes_get_post_categories( $arg = array() ) {
		...
		if ( $categories ) {
			...
			foreach ( $categories as $cat ) {
				if ( ! $no_link ) $categories_html .= '<a href="' . esc_url_raw( get_term_link( $cat->term_id ) ) . '" class="btArticleCategory ' . $cat->slug . '">';
		...
		if ( ! is_404() && ! is_front_page() ) {
			...
			if ( ! $simple ) {
				...
				if ( ! is_singular() || is_page() ) {
					$output .= '<span><a href="' . esc_url_raw( $home_link ) . '">' . esc_html__( 'Home', 'law-firm' ) . '</a></span>';
				}
			} else {
				if ( ! is_singular() || is_page() ) {
					$output .= '<a href="' . esc_url_raw( $home_link ) . '">' . esc_html__( 'Home', 'law-firm' ) . '</a>';
				}
			}
			...
			if ( is_home() ) {
				...
				if ( !is_null( boldthemes_get_option('pf_slug') ) && boldthemes_get_option('pf_slug') != "" && !is_null( boldthemes_get_id_by_slug( boldthemes_get_option('pf_slug') ) ) && boldthemes_get_id_by_slug( boldthemes_get_option('pf_slug') ) != '' ) {
				...
				$output .= $item_prefix . esc_html__( 'Portfolio', 'law-firm' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Category', 'law-firm' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Tag', 'law-firm' ) . $item_suffix;
				
				$subtitle = '';
		  
			} else if ( is_author() ) {
			
				$output .= $item_prefix . esc_html__( 'Author', 'law-firm' ) . $item_suffix;
				...
				$output .= $item_prefix . esc_html__( 'Search', 'law-firm' ) . $item_suffix;
