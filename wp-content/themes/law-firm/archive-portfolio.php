...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		} else if ($pf_list_view == 'simple' ) {
			get_template_part( 'views/portfolio/list/simple' );
		} else {
	...
	if ( is_search() ) { ?>
		<article class="btNoSearchResults bt_bb_section bt_bb_top_spacing_large bt_bb_bottom_spacing_large bt_bb_layout_boxed_1200">
			<div class="bt_bb_port">
				<div class="bt_bb_cell">
				<?php 
				echo boldthemes_get_heading_html(
					array(
						'headline' => esc_html__( 'We are sorry, no results for: ', 'law-firm' ) . get_search_query(),
						'subheadline' => esc_html__( 'Back to homepage', 'law-firm' ),
						'url' => site_url(),
						'size' => 'medium'
					)									 
				);
